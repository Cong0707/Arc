...
public class Preloader {
	...
	public void preload (final String assetFileUrl, final PreloaderCallback callback) {
		...
		loader.loadText(baseUrl + assetFileUrl + "?etag=" + System.currentTimeMillis(), new AssetDownloader.AssetLoaderListener<String>() {
			@Override
	...
	public void preloadSingleFile (final String file) {
		...
		Log.infoTag("Preloader", "Downloading " + baseUrl + asset.file);
	...
	public Fi[] list (final String file) {
	...
	public Fi[] list (final String file, final FileFilter filter) {
		return getMatchedAssetFiles(new FilePathFilter() {
			@Override
			public boolean accept (String path) {
	...
	public Fi[] list (final String file, final FilenameFilter filter) {
		return getMatchedAssetFiles(new FilePathFilter() {
			@Override
			public boolean accept (String path) {
	...
	public Fi[] list (final String file, final String suffix) {
		return getMatchedAssetFiles(new FilePathFilter() {
			@Override
			public boolean accept (String path) {
	...
	private Fi[] getMatchedAssetFiles (FilePathFilter filter) {
		Seq<Fi> files = new Seq<>();
		for (String file : assetNames.keys()) {
			if (filter.accept(file)) {
				files.add(new GwtFi(this, file, Files.FileType.internal));
			}
		}

		Fi[] filesArray = new Fi[files.size];
