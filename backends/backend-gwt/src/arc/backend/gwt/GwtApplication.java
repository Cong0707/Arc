...
public abstract class GwtApplication implements EntryPoint, Application {
	...
	protected final Seq<Runnable> runnables = new Seq<Runnable>();
	protected final Seq<Runnable> runnablesHelper = new Seq<Runnable>();
	protected final Seq<ApplicationListener> lifecycleListeners = new Seq<ApplicationListener>();
	private int lastWidth;
	private int lastHeight;
	Preloader preloader;
	private static AgentInfo agentInfo;
	private GwtClipboard clipboard;
	...
	void setupLoop () {
		...
		try {
			...
			root.clear();
			Log.errTag("GwtApplication", "exception: " + e.getMessage());
			e.printStackTrace();
		...
		try {
			...
			Log.errTag("GwtApplication", "exception: " + t.getMessage());
			t.printStackTrace();
	...
	public void removeListener (ApplicationListener listener) {
		synchronized (lifecycleListeners) {
			lifecycleListeners.remove(listener, true);
		}
	}
	...
	private void onVisibilityChange (boolean visible) {
		if (visible) {
			for (ApplicationListener listener : lifecycleListeners) {
				listener.resume();
			}
			listener.resume();
		} else {
