...
public class AsyncExecutor implements Disposable {
	...
	public <T> arc.util.async.AsyncResult<T> submit (final arc.util.async.AsyncTask<T> task) {
		T result = null;
		try {
			result = task.call();
		} catch (Throwable t) {
		...
		return new arc.util.async.AsyncResult(result);
	}

	/** Waits for running {@link arc.util.async.AsyncTask} instances to finish, then destroys any resources like threads. Can not be used after
