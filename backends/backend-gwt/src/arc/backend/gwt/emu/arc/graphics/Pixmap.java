...
public class Pixmap implements Disposable {
	...
	public static Pixmap createFromFrameBuffer (int x, int y, int w, int h) {
		...
		ByteBuffer pixels = Buffers.newByteBuffer(h * w * 4);
	...
	public Pixmap (Fi file) {
		this(((GwtFi)file).preloader.images.get(file.path()));
		if (imageElement == null) throw new ArcRuntimeException("Couldn't load image '" + file.path() + "', file does not exist");
	}
