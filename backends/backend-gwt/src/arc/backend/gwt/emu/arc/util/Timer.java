 ...
 * @author Nathan Sweet */
public class Timer {
	static final Seq<Timer> instances = new Seq(1);
	...
	public Task scheduleTask (Task task, float delaySeconds, float intervalSeconds, int repeatCount) {
		...
		task.executeTimeMillis = Time.nanos() / 1000000 + (long)(delaySeconds * 1000);
	...
	static class TimerThread extends com.google.gwt.user.client.Timer implements Runnable, ApplicationListener {
		...
		public void run () {
			synchronized (instances) {
				...
				long timeMillis = Time.nanos() / 1000000;
		...
		public void resume () {
			long delayMillis = Time.nanos() / 1000000 - pauseMillis;
			synchronized (instances) {
				for (int i = 0, n = instances.size; i < n; i++)
		...
		public void pause () {
			pauseMillis = Time.nanos() / 1000000;
			synchronized (instances) {
				app = null;
				wake();
			}
