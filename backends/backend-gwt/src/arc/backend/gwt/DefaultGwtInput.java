...
public class DefaultGwtInput extends GwtInput {
	...
	public DefaultGwtInput (CanvasElement canvas, GwtApplicationConfiguration config) {
		pressedKeys = new boolean[KeyCode.all.length + 1];
		justPressedKeys = new boolean[KeyCode.all.length + 1];
		this.canvas = canvas;
		this.config = config;
		...
		setCatch(KeyCode.backspace, true);
	}
	...
	private KeyCode getButton (int button) {
		if (button == NativeEvent.BUTTON_LEFT) return KeyCode.mouseLeft;
	...
	private void handleEvent (NativeEvent e) {
		if (e.getType().equals("mousedown")) {
			...
			this.touched[0] = true;
			final int button = getButton(e.getButton()).ordinal();
			...
			this.currentEventTimeStamp = Time.nanos();
		...
		if (hasFocus && !e.getType().equals("blur")) {
			if (e.getType().equals("keydown")) {
				...
				KeyCode code = keyForCode(e.getKeyCode(), getKeyLocationJSNI(e));
				if (isCatch(code)) {
					e.preventDefault();
				}
			...
			if (e.getType().equals("keyup")) {
				...
				if (processor != null && code == KeyCode.tab) {
			...
			IntSet.IntSetIterator iterator = pressedKeySet.iterator();
			...
			while (iterator.hasNext) {
				...
				if (processor != null) {
					processor.keyUp(keyForCode(code, LOCATION_STANDARD));
				}
			}
		}
		...
		if (e.getType().equals("touchstart")) {
			...
			for (int i = 0, j = touches.length(); i < j; i++) {
				...
				deltaY[touchId] = 0;
				if (processor != null) {
					processor.touchDown(touchX[touchId], touchY[touchId], touchId, KeyCode.mouseLeft);
				}
			}
			this.currentEventTimeStamp = Time.nanos();
			e.preventDefault();
		...
		if (e.getType().equals("touchcancel")) {
			...
			for (int i = 0, j = touches.length(); i < j; i++) {
				...
				touchY[touchId] = getRelativeY(touch, canvas);
				if (processor != null) {
					processor.touchUp(touchX[touchId], touchY[touchId], touchId, KeyCode.mouseLeft);
				}
			}
			this.currentEventTimeStamp = Time.nanos();
			e.preventDefault();
...
// if(hasFocus) e.preventDefault();
	...
	protected KeyCode keyForCode (int keyCode, int location) {
		switch (keyCode) {
		case KeyCodes.KEY_ALT:
			return location == LOCATION_RIGHT ? KeyCode.altRight : KeyCode.altLeft;
		case KeyCodes.KEY_BACKSPACE:
			return KeyCode.backspace;
		case KeyCodes.KEY_CTRL:
			return location == LOCATION_RIGHT ? KeyCode.controlRight : KeyCode.controlLeft;
		...
		case KeyCodes.KEY_ENTER:
			return location == LOCATION_NUMPAD ? KeyCode.dpadCenter : KeyCode.enter;
		...
		case KeyCodes.KEY_SHIFT:
			return location == LOCATION_RIGHT ? KeyCode.shiftRight : KeyCode.shiftLeft;
