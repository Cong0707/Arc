 ...
 * 
 * @author barkholt */
public class WebAudioAPISound extends Sound {
	...
	public void setAudioBuffer (JavaScriptObject audioBuffer) {
		...
		IntMap.Keys keys = activeSounds.keys();
		while (keys.hasNext) {
			int key = keys.next();
			pause(key);
			resume(key, 0f);
	...
	public native JavaScriptObject createBufferSourceNode (boolean loop, float pitch) /*-{
		...
		var audioContext = this.@arc.backend.gwt.webaudio.WebAudioAPISound::audioContext;
		var audioBuffer = this.@arc.backend.gwt.webaudio.WebAudioAPISound::audioBuffer;
	...
	public native JavaScriptObject playJSNI (JavaScriptObject source, int key, float startOffset) /*-{
		...
		source.onended = function() {
			self.@arc.backend.gwt.webaudio.WebAudioAPISound::soundDone(I)(key);
		};
