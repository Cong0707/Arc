...
object VulkanUtil {
    fun asPointerBuffer(collection: Collection<String?>): PointerBuffer {
        val stack = stackGet()

        val buffer = stack.mallocPointer(collection.size)

        collection.stream()
            .map { text: String? -> stack.UTF8(text) }
            .forEach { buffer: ByteBuffer -> buffer.put(buffer) }

        return buffer.rewind()
    }

    fun checkResult(result: Int, errorMessage: String?) {
        if (result != VK_SUCCESS) {
            throw ArcRuntimeException(java.lang.String.format("%s: %s", errorMessage, translateVulkanResult(result)))
        }
    }

    fun translateVulkanResult(result: Int): String { //move to arc.graphics soon
        return when (result) {
            ...
            VK_ERROR_INCOMPATIBLE_DISPLAY_KHR -> ("The display used by a swapchain does not use the same presentable image layout, or is incompatible in a way that prevents sharing an"
                    + " image.")
